...
def  generate_data(datadir, dataset='ml-1m', split_way='threshold', threshold=30, supp_ratio=None, training_ratio=1):
    n_user = config[dataset]['n_user']
    n_item = config[dataset]['n_item']
    with open("cur_10_support_as_core.pkl", 'rb') as f:
        train_set_supp = pickle.load(f)
        train_set_que = pickle.load(f)
        test_set_supp = pickle.load(f)
        test_set_que = pickle.load(f)
        user_supp_list = pickle.load(f)
        user_his_dic = pickle.load(f)
    # print("TYPE OF USER SUPPORT LIST:", type(user_supp_list))
    # print("USER SUPPORT LIST:", user_supp_list)
# def generate_data(datadir, dataset='ml-1m', split_way='threshold', threshold=30, supp_ratio=None, training_ratio=1):
#     n_user = config[dataset]['n_user']
#     n_item = config[dataset]['n_item']
